...
with c1:
        fig = px.bar(df, x='Sender', y='Transfers Volume', color='Sending Volume', title='Top 20 Senders based on Sending Volume')
        fig.update_layout(legend_title=None, xaxis_title=None, yaxis_title='$NEAR')
        st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
        
df = Top_20_Receivers_based_on_Receiving_Volume
with c2:
        fig = px.bar(df, x='Receiver', y='Transfers Volume', color='Receiving Volume', title='Top 20 Receivers based on Receiving Volume')
        fig.update_layout(legend_title=None, xaxis_title=None, yaxis_title='$NEAR')
        st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
